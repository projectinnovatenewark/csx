...
# FIXME: Change the variable of num = 15 so that a variable of num takes in a user input. 
# FIXME: num_analyzer should test every number from the user input all the way down to on.

def isPrime(n):
    """isPrime should take a number as an argument and return true if it is prime number or false if it is not"""
    truth_val = True
...
def num_analyzer(): #argument needed in function definition
    """num_analyzer should accept an argument and return if it is an even number or false AND if it is prime
    it should test from every number """
